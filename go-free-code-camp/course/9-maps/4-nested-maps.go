...
func getNameCounts(names []string) map[rune]map[string]int {
	...
	for _, name := range names {
		if name == "" {
			continue // não importamos com nomes vazios
		}

		firstChar := rune(name[0]) // pegando primeiro caractere do nome, esse vai ser a rune
		_, ok := counts[firstChar]
		if !ok {
			counts[firstChar] = make(map[string]int) // se esse caractere ainda não existir no map, vamos inicializar ele
		}
		// nesse ponto, já existe um map inicializado com a letra especifica
		counts[firstChar][name]++ // se o nome da key não existir vai ser 0, então vai incrementar e salvar no map
	}
